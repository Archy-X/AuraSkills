...
public class BlockSourceParser extends SourceParser<BlockSource> {
    ...
    public BlockSource parse(ConfigurationNode source, ConfigurateSourceContext context) throws SerializationException {
        ...
        boolean checkReplaceAllStates = source.node("check_replace_all_states").getBoolean(false);
        ...
        return new BlockSource(plugin, context.parseValues(source), blocks, maxBlocks, triggers, checkReplace, checkReplaceAllStates, states, afterStates, stateMultiplier, supportBlockType, allowBoneMeal);
    }
