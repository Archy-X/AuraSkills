...
public abstract class LootLoader {
    ...
    private LootTable loadLootTable(NamespacedId id, File file, ConfigurationNode config) {
        ...
        for (ConfigurationNode poolNode : poolsNode.childrenMap().values()) {
            ...
            for (ConfigurationNode lootNode : poolNode.node("loot").childrenList()) {
                ...
                try {
                    ...
                    if (lootType == LootType.ITEM) {
                        ...
                        loot = getParser(lootType).parse(lootId, context, ApiConfigNode.toApi(lootNode), manager.getLootRequirements(lootNode));
                        ...
                        loot = customParser.parse(lootId, context, ApiConfigNode.toApi(lootNode), manager.getLootRequirements(lootNode));
                    }
                } catch (Exception e) {
            ...
            // Create pool
            LootPool pool = new LootPool(poolId, poolName, lootList, baseChance, selectionPriority, overrideVanillaLoot, options, manager.getLootRequirements(poolNode));
            pools.add(pool);
        }
