...
public class ManaCommand extends BaseCommand {
    ...
    public void onManaAdd(CommandSender sender, @Flags("other") Player player, int amount, @Default("true") boolean allowOverMax) {
        Locale locale = Lang.getLanguage(player);
        ManaManager mana = AureliumSkills.manaManager;
        UUID id = player.getUniqueId();
        if (amount >= 0) {
    ...
    public void onManaRemove(CommandSender sender, @Flags("other") Player player, int amount) {
        Locale locale = Lang.getLanguage(player);
        ManaManager mana = AureliumSkills.manaManager;
        UUID id = player.getUniqueId();
        if (amount >= 0) {
    ...
    public void onManaSet(CommandSender sender, @Flags("other") Player player, int amount, @Default("true") boolean allowOverMax) {
        Locale locale = Lang.getLanguage(player);
        ManaManager mana = AureliumSkills.manaManager;
        UUID id = player.getUniqueId();
        if (amount >= 0) {
