...
public abstract class SkillLevelItem extends AbstractItem implements TemplateItemProvider<Integer> {
    ...
    @Override
    public @NotNull SlotPos getSlotPos(Player player, ActiveMenu activeMenu, Integer context) {
        int index = context - 2;
    ...
    @Override
    public @Nullable ItemStack onItemModify(@NotNull ItemStack baseItem, Player player, @NotNull ActiveMenu activeMenu, Integer num) {
    ...
    protected @Nullable String getRewardsLore(Skill skill, int level, Player player, Locale locale) {
