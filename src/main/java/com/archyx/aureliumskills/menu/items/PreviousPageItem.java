...
public class PreviousPageItem implements ConfigurableItem {
    ...
    public ItemStack getItem(Locale locale) {
        ...
        if (meta != null) {
            meta.setDisplayName(displayName.replace("{previous_page}", Lang.getMessage(MenuMessage.PREVIOUS_PAGE, locale)));
            ...
            for (int i = 0; i < lore.size(); i++) {
                ...
                for (String placeholder : placeholders) {
                    if (placeholder.equals("previous_page_click")) {
                        line = line.replace("{previous_page_click}", Lang.getMessage(MenuMessage.PREVIOUS_PAGE_CLICK, locale));
                    }
                }
                builtLore.add(line);
            }
